...
<h4>Flashcard: {{ $flashcard->title }}</h4>
<h4>Categoria: {{ $flashcard->category->name }}</h4>

@foreach ($practices as $item)
    <div class="card">
        <div class="card-body">

            <!-- Cabeçalho com data e nível -->
            <p class="small text-muted d-flex">
                <span>{{ $item->created_at ? \Carbon\Carbon::parse($item->created_at)->format('d/m/Y H:i') : '' }}</span>
                <span class="badge">
                    @switch($item->level)
                        @case('very_easy')
                            Muito fácil
